...
inline value_type const *logic_and_node::infer(type_storage const &types) const
{
    return types.get(lhs).type->logic_and(types.get(rhs).type);
}

// TODO: emit the correct `add` depending on the type
inline entt::entity logic_and_node::emit(builder &bld, value_type const *ty) const
{
    if (ty->is_const())
