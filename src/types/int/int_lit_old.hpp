...
struct int_top final : int_value
{
    ...
    inline value const *lsh(value const *rhs) const noexcept
    {
        return rhs->as<int_value>()
                   ? (value const *)this
                   : value::lsh(rhs);
    ...
    inline value const *rsh(value const *rhs) const noexcept
    {
        return rhs->as<int_value>()
                   ? (value const *)this
                   : value::rsh(rhs);
...
struct int_lit final : int_value
{
    inline static int_lit const *make(int64_t n) noexcept
    {
        // TODO: use something more optimal, remove caching once you get "in-place" types
        static entt::dense_map<int64_t, std::unique_ptr<int_lit const>, std::identity> cache;
        if (auto iter = cache.find(n); iter != cache.end())
            return iter->second.get();
        return cache.insert({n, std::unique_ptr<int_lit>(new int_lit{n})}).first->second.get();
    }
    ...
    inline value const *add(value const *rhs) const noexcept
    {
        if (rhs->as<int_top>())
            return rhs;
        else if (auto ptr = rhs->as<int_lit>(); ptr)
            return int_lit::make(n + ptr->n);
    ...
    inline value const *div(value const *rhs) const noexcept
    {
        if (rhs->as<int_top>())
            ...
            return (ptr->n == 0) ? int_bot::self() : int_lit::make(n / ptr->n);
        else if (rhs->as<int_bot>())
            return this;
        else
    ...
    inline value const *neg() const noexcept { return int_lit::make(-n); }
...
private:
    explicit int_lit(int64_t n) noexcept : n{n} {}
};
...
inline value const *sint_type::zero() const noexcept { return int_lit::make(0); }